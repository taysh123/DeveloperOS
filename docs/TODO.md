...
## Post-roadmap: Slice 15 — Windows installer (desktop ladder step D) ✅
- [x] `packaging/installer.iss` (Inno Setup; dev-time tool via winget): **per-user** (`PrivilegesRequired=lowest` → `%LOCALAPPDATA%\Programs\DeveloperOS`, no admin/UAC), Start-Menu shortcut + opt-in desktop icon, LICENSE page, branded `devos.ico`, output `DeveloperOS-Setup-<version>.exe` (~11 MB), **KEEP-USER-DATA** uninstall (no `[UninstallDelete]` — `%APPDATA%\DeveloperOS` survives by design). `build_installer.ps1` reads the version from `devos/__init__.py`, builds the exe if missing, locates ISCC.
- [x] **Update strategy = manual by design** (documented in packaging/README): GitHub Releases page; newer Setup upgrades in place (stable AppId); no auto-update code, no new network surface. Unsigned-binary SmartScreen caveat documented. **Live-verified full cycle** (silent install → installed exe serves dashboard → silent uninstall; sentinel data survived). New root `LICENSE` (MIT). 356 tests (+5). D-0032; SECURITY unchanged.
...
## Post-roadmap: Slice 12 — Desktop strategy + PWA foundation ✅
- [x] **Desktop strategy decided (D-0029):** hybrid **PWA front + packaged Python backend** ladder — A) PWA foundation ✅ → B) `devos app` launcher → C) PyInstaller single-file `devos.exe` → D) Inno Setup installer + optional manual update check (GitHub Releases; offline always works) → E) Tauri **only if justified**. **Electron rejected** (~200 MB Chromium + Node toolchain vs stdlib-only ethos). Windows-first, cross-platform-friendly.
- [x] **PWA foundation shipped:** `static/manifest.webmanifest` (name DeveloperOS / short_name DevOS, start_url+scope "/", `display: standalone`, theme/background `#0f1117`, 192/512 + maskable + apple-touch icons); brand mark `>_` (accent on panel color) generated by **stdlib-only** `tools/make_icons.py` (zlib/struct PNG writer; PNGs committed/vendored) + hand-written `icons/favicon.svg`; index.html head wiring; `.png`/`.webmanifest` added to `_CONTENT_TYPES` (only backend change). **No service worker by design; zero new endpoints; SECURITY unchanged.** 340 tests (+6 contract tests), live socket smoke verified. Dashboard installs from Edge/Chrome as a desktop app.
