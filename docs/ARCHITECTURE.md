...
The dashboard is an **installable PWA** (D-0029): `static/manifest.webmanifest` + committed icons
(generated by stdlib-only `tools/make_icons.py`); deliberately **no service worker** (the backend
is local). Desktop distribution follows the D-0029 ladder (next: `devos app` launcher, then a
PyInstaller `devos.exe`).
